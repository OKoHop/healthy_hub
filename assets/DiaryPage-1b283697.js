import{s as i,j as e,N as V,m as Q,r as W,a as c}from"./index-38fd0f65.js";import{s as D,a as J,b as L,c as v,d as Y}from"./statisticSelectors-24f8197b.js";const I="data:image/png;base64,",O="data:image/png;base64,",K="data:image/png;base64,",P="data:image/png;base64,";let d=(n=21)=>crypto.getRandomValues(new Uint8Array(n)).reduce((s,t)=>(t&=63,t<36?s+=t.toString(36):t<62?s+=(t-26).toString(36).toUpperCase():t>62?s+="-":s+="_",s),"");const q=i.div`
  display: flex;
  row-gap: 12px;
  column-gap: 20px;
  flex-wrap: wrap;

  font-size: ${n=>n.theme.fontSizes.xs};
  font-weight: ${n=>n.theme.fontWeights.regular};
  line-height: 20px;

  @media screen and (${n=>n.theme.mq.tablet}) {
    margin-right: 60px;
    flex-wrap: nowrap;
  }

  @media screen and (${n=>n.theme.mq.desktop}) {
    margin-right: 25px;
  }
`,M=i.p`
  width: 100%;

  @media screen and (${n=>n.theme.mq.tablet}) {
    width: 164px;
  }
`,H=i.p`
  width: 92px;
`,U=i.p`
  width: 66px;
`,B=i.span`
  color: ${n=>n.theme.colors.gray};
  font-weight: ${n=>n.theme.fontWeights.medium};
`,x=({info:n})=>{const{carbonohidrates:s,protein:t,fat:o}=n;return e.jsxs(q,{children:[e.jsxs(M,{children:["Carbonohidrates: ",e.jsx(B,{children:s})]}),e.jsxs(H,{children:["Protein: ",e.jsx(B,{children:t})]}),e.jsxs(U,{children:["Fat: ",e.jsx(B,{children:o})]})]})},G="/healthy_hub/assets/edit-2-cb983d1b.svg",C=i.li`
  display: flex;
  column-gap: 16px;
  color: ${n=>n.theme.colors.white};
  font-family: ${n=>n.theme.fontFamily.poppins};
  font-size: ${n=>n.theme.fontSizes.xs};
  font-weight: ${n=>n.theme.fontWeights.regular};
  line-height: 20px;
`,y=i.span`
  min-width: 16px;
  font-weight: ${n=>n.theme.fontWeights.semiBold};
  text-align: center;
`,T=i.div`
  display: flex;
  width: 100%;
  flex-wrap: wrap;
  row-gap: 6px;
`,Z=i.p`
  text-transform: capitalize;
  flex-grow: 1;
`,$=i.button`
  padding: 0;
  border: none;
  background-color: transparent;
  display: flex;
  align-items: center;
  column-gap: 6px;
  color: ${n=>n.theme.colors.gray};
  font-family: ${n=>n.theme.fontFamily.poppins};
  font-size: ${n=>n.theme.fontSizes.xs};
  font-weight: ${n=>n.theme.fontWeights.regular};
  line-height: 20px;
  cursor: pointer;

  @media screen and (${n=>n.theme.mq.tablet}) {
    order: 1;
  }
`,_=i.img`
  width: 16px;
  height: 16px;
`,ee=i.div`
  display: flex;
  column-gap: 12px;
  font-size: 12px;
  line-height: 18px;

  @media screen and (${n=>n.theme.mq.tablet}) {
    font-size: ${n=>n.theme.fontSizes.xs};
    line-height: 20px;
  }
`,F=i.p`
  display: flex;
  width: 70px;
  column-gap: 6px;

  & > span {
    color: ${n=>n.theme.colors.gray};
  }
  
  @media screen and (${n=>n.theme.mq.tablet}) {
    width: 80px;

    & > span {
      display: none;
    }
  }
`,ne=i(F)`
  width: 62px;
  @media screen and (${n=>n.theme.mq.tablet}) {
    width: 80px;
  }
`,ie=i(F)`
  width: 58px;
  @media screen and (${n=>n.theme.mq.tablet}) {
    width: 80px;
  }
`,se=i.button`
  display: flex;
  align-items: center;
  column-gap: 8px;
  color: ${n=>n.theme.colors.greenLite};
  font-family: ${n=>n.theme.fontFamily.poppins};
  font-size: ${n=>n.theme.fontSizes.xs};
  font-weight: ${n=>n.theme.fontWeights.medium};
  line-height: 20px;
  background-color: transparent;
  border: none;
  outline: none;
  padding: 0;
  cursor: pointer;
`,h=({item:n,index:s})=>{const{carbohydrate:t,protein:o,fat:u,mealName:S,mealType:X,showButton:k}=n;return e.jsx(e.Fragment,{children:k?e.jsxs(C,{children:[e.jsx(y,{children:s+1}),e.jsx(se,{onClick:!0,children:"+ Record your meal"})]}):e.jsxs(C,{children:[e.jsx(y,{children:s+1}),X&&e.jsxs(T,{children:[e.jsx(Z,{children:S}),e.jsxs($,{type:"button",onClick:!0,children:[e.jsx(_,{src:G,alt:"Edit"}),"Edit"]}),e.jsxs(ee,{children:[e.jsxs(F,{children:[e.jsx("span",{children:"Carb."}),t]}),e.jsxs(ne,{children:[e.jsx("span",{children:"Prot."}),o]}),e.jsxs(ie,{children:[e.jsx("span",{children:"Fat."}),u]})]})]})]})})},p=(n,s)=>n.length<=3?[...n,...Array(1).fill({mealType:s,showButton:!0}),...Array(3-n.length).fill({})]:[...n,...Array(1).fill({mealType:s,showButton:!0})],te="/healthy_hub/assets/arrow-right-3924980b.svg",re=i.main`
  background-color: #050505;
  color: #FFFFFF;
  padding-top: 16px;
  padding-bottom: 60px;

  @media screen and (min-width: 834px) {
    padding-top: 24px;
    padding-bottom: 20px;
  }
  @media screen and (min-width: 1440px) {
    padding-top: 20px;
  }
`,ae=i.div`
  margin: 0 auto;
  width: 320px;
  padding: 0 10px;

  @media screen and (min-width: 834px) {
    width: 834px;
    padding: 0 27px;
  }
  @media screen and (min-width: 1440px) {
    width: 1440px;
    padding: 0 34px;
  }
`,oe=i(V)`
  display: flex;
  @media screen and (min-width: 320px) {
    margin-right: 8px;
  }
  @media screen and (min-width: 834px) {
    margin-right: 12px;
  }
`,ce=i.img`
  @media screen and (min-width: 320px) {
    width: 16px;
  }
  @media screen and (min-width: 834px) {
    width: 24px;
  }
`,de=i.header`
  display: flex;
  align-items: center;
  margin-bottom: 20px;

  @media screen and (min-width: 834px) {
    margin-bottom: 24px;

  }
  @media screen and (min-width: 1440px) {
    margin-bottom: 16px;
  }
`,xe=i.h1`
  font-size: 24px;
  font-weight: 500;
  line-height: 1.25;

  @media screen and (min-width: 834px) {
    font-size: 30px;
    line-height: 1.2;
  }
`,he=i.ul`
  display: flex;
  flex-direction: column;
  gap: 24px;

  @media screen and (min-width: 1440px) {
    flex-direction: row;
    flex-wrap: wrap;
    gap: 20px;

    & > li:nth-child(2) {
      order: 2;
    }

    & > li:nth-child(3) {
      order: 1;
    }

    & > li:nth-child(4) {
      order: 3;
    }
  }
`,l=i.li`
  display: flex;
  flex-direction: column;
  gap: 12px;

  @media screen and (min-width: 834px) {
    gap: 6px;
  }

  @media screen and (min-width: 1440px) {
    width: calc((100% - 20px) / 2);
  }
`,g=i.div`
  @media screen and (min-width: 834px) {
    display: flex;
    align-items: center;
    justify-content: space-between;
  }
`,m=i.div`
  display: flex;
  align-items: center;
  gap: 12px;
  margin-bottom: 12px;

  @media screen and (min-width: 834px) {
    margin-bottom: 0;
  }

`,A=i.div`
  width: 32px;
  height: 32px;
  border-radius: 50%;
`,j=i.img`
  display: block;
  height: 100%;
  width: 100%;
  object-fit: cover;
`,f=i.h2`
  font-size: 18px;
  line-height: 26px;

  @media screen and (min-width: 834px) {
    font-size: 22px;
    line-height: 32px;
  }
`,w=i.ul`
  background-color: #0f0f0f;
  border-radius: 12px;
  height: 232px;
  padding: 16px 12px;

  @media screen and (min-width: 834px) {
    height: 240px;
    padding: 32px 14px;
  }
`,b=i.ul`
  display: flex;
  flex-direction: column;
  row-gap: 40px;
  height: 200px;
  overflow-y: auto;
  padding-right: 10px;

  &::-webkit-scrollbar {
    width: 7px;
  }

  &::-webkit-scrollbar-thumb {
    background-color: rgba(41, 41, 40, 0.2);
    border-radius: 12px;
    border: 1px solid rgba(227, 255, 168, 0.2);

    &:hover {
      background-color: rgba(227, 255, 168, 0.2);
    }
  }

  @media screen and (min-width: 834px) {
    height: 176px;
    row-gap: 32px;
  }
`,ge=()=>{var R;const n=Q(),s=W.useRef(((R=n.state)==null?void 0:R.from)??"/main"),t=c(D),o=c(J),u=c(L),S=c(v),X=c(Y),{breakfast:k,lunch:E,dinner:z,snack:N}=t;return e.jsx(re,{children:e.jsxs(ae,{children:[e.jsxs(de,{children:[e.jsx(oe,{to:s.current,children:e.jsx(ce,{src:te,alt:"arrow right"})}),e.jsx(xe,{children:"Diary"})]}),e.jsxs(he,{children:[e.jsxs(l,{children:[e.jsxs(g,{children:[e.jsxs(m,{children:[e.jsx(A,{children:e.jsx(j,{src:I,alt:"Breakfast on a plate"})}),e.jsx(f,{children:"Breakfast"})]}),e.jsx(x,{info:k})]}),e.jsx(w,{children:e.jsx(b,{children:p(o,"Breakfast").map((r,a)=>e.jsx(h,{item:r,index:a,img:I},d()))})})]}),e.jsxs(l,{children:[e.jsxs(g,{children:[e.jsxs(m,{children:[e.jsx(A,{children:e.jsx(j,{src:K,alt:"Lunch on a plate"})}),e.jsx(f,{children:"Lunch"})]}),e.jsx(x,{info:E})]}),e.jsx(w,{children:e.jsx(b,{children:p(u,"Lunch").map((r,a)=>e.jsx(h,{item:r,index:a,img:K},d()))})})]}),e.jsxs(l,{children:[e.jsxs(g,{children:[e.jsxs(m,{children:[e.jsx(A,{children:e.jsx(j,{src:O,alt:"Dinner on a plate"})}),e.jsx(f,{children:"Dinner"})]}),e.jsx(x,{info:z})]}),e.jsx(w,{children:e.jsx(b,{children:p(S,"Dinner").map((r,a)=>e.jsx(h,{item:r,index:a,img:O},d()))})})]}),e.jsxs(l,{children:[e.jsxs(g,{children:[e.jsxs(m,{children:[e.jsx(A,{children:e.jsx(j,{src:P,alt:"Snack on a plate"})}),e.jsx(f,{children:"Snack"})]}),e.jsx(x,{info:N})]}),e.jsx(w,{children:e.jsx(b,{children:p(X,"Snack").map((r,a)=>e.jsx(h,{item:r,index:a,img:P},d()))})})]})]})]})})};export{ge as default};
